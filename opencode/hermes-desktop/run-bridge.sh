...
#   OPENCODE_CONFIG / OPENCODE_CONFIG_DIR   read-only guardrails for the OpenCode agent
#                                          (point both at ../../pi-agents/macos/opencode — see there)
...
LABEL="${OPENCODE_BRIDGE_LABEL:-com.hermes.opencode-bridge}"   # launchd label / systemd unit stem (override to run a 2nd instance)
...
print_config() {
  ...
  case "${OPENCODE_CONFIG:-}" in
    *pure-llm.json) echo "  guardrails:     pure-LLM config (OpenCode has no tools; pi enforces approvals on its side)" ;;
    *) echo "  guardrails:     ${OPENCODE_CONFIG_DIR:-none (agent can write/delete — see ../../pi-agents/macos/opencode)}" ;;
  esac
}
...
write_service_runner() {
  ...
  build_argv
  {
    ...
    # Read-only guardrails for the OpenCode agent itself (see ../../pi-agents/macos/opencode):
    # OPENCODE_CONFIG = permission policy, OPENCODE_CONFIG_DIR = directory with plugins/.
    # Baked here because the runner is exec'd by launchd/systemd with a bare environment.
    if [ -n "${OPENCODE_CONFIG:-}" ];     then printf 'export OPENCODE_CONFIG=%s\n' "$(shell_quote "$OPENCODE_CONFIG")"; fi
    if [ -n "${OPENCODE_CONFIG_DIR:-}" ]; then printf 'export OPENCODE_CONFIG_DIR=%s\n' "$(shell_quote "$OPENCODE_CONFIG_DIR")"; fi
    if [ -n "${PI_GUARDRAILS_CONFIG:-}" ]; then printf 'export PI_GUARDRAILS_CONFIG=%s\n' "$(shell_quote "$PI_GUARDRAILS_CONFIG")"; fi
    if [ -n "${PI_GUARDRAILS_AUDIT:-}" ];  then printf 'export PI_GUARDRAILS_AUDIT=%s\n' "$(shell_quote "$PI_GUARDRAILS_AUDIT")"; fi
    printf 'exec'
    local arg
