#!/usr/bin/env python3
"""claude_code_bridge.py — ClaudeCode-compatible bridge from Hermes to the Claude Code CLI.

Invokes the Claude Code CLI through a chat-completions HTTP interface.
It fulfils each request by invoking an already-authenticated ``claude -p`` process,
exposing it over a chat-completions HTTP surface that Hermes (or any
chat-completions client) can point a custom provider at.

What it does better than the old proxy shim:
  * Real usage + cost — parses ``claude -p --output-format json`` and reports the
    actual input/output/cache tokens and ``total_cost_usd`` in the chat-completions ``usage``
    block and the access log (the old shim always returned zeros).
  * Real streaming — ``--output-format stream-json`` events are translated to
    incremental chat-completions SSE ``chat.completion.chunk`` deltas as Claude produces
    them (the old shim ran to completion, then emitted three fake chunks).
  * Concurrency control — a bounded semaphore caps how many ``claude`` subprocesses
    run at once, with a fast 429 when the queue is saturated, so a burst of
    requests can't fork-bomb the host.
  * Correct roles — ``system`` messages become ``--append-system-prompt`` instead
    of being flattened into the user prompt.
...
Stdlib only; no third-party deps, so it stays trivially portable into a slim
container or onto a developer host.
"""
...
# configured MCP tools). Lock it down per-deployment with --disallowed-tools
...
def split_messages(messages: list[dict[str, Any]]) -> tuple[str, str]:
    ...
    turns become the conversational prompt. Unlike the old shim, the system role
    is not dumped into the prompt body.
    """
...
def _popen(cmd: list[str], cfg: "BridgeConfig") -> subprocess.Popen:
    ...
    return subprocess.Popen(
        ...
        env=getattr(cfg, "child_env", None),
        **kwargs,
    )
...
def run_blocking(cfg: "BridgeConfig", model: str, prompt: str, system_prompt: str) -> dict[str, Any]:
    ...
    # Verbose JSON includes the native tool events followed by the final result.
    try:
    ...
    if not results or results[-1].get("is_error"):
        raise ClaudeError("Claude Code did not produce a successful result", 502)
    result = results[-1]
    if result.get("subtype") != "success":
        raise ClaudeError("Claude Code stopped before successful completion", 502)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--pass-model", action="store_true")
    return p
...
def _selfcheck() -> None:
    # ponytail: offline checks for the two bits with real logic — message
