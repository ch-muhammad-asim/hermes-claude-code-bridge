...
| pi auto-compacts far too early (status bar `?/200k` on a 1M model) | The bridge must advertise `context_length` on `/v1/models`; check with `curl -s http://127.0.0.1:$PORT/v1/models`. `null` means you are on an older bridge build — or on someone else's, per the row above |
