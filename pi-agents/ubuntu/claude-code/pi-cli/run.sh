...
write_runner() {
  ...
  [ -n "$npm_bin" ] && npm_bin="$npm_bin/bin"
  {
    ...
    # npm's global bin goes FIRST: dirname(python3) is /usr/bin on most distros, and a stale
    # copy of the CLI installed there would shadow the current npm one for the whole service.
    printf 'export PATH=%s:%s:%s/.local/bin:/usr/local/bin:/usr/bin:/bin\n' \
      "$(shell_quote "${npm_bin:-/usr/local/bin}")" "$(shell_quote "$(dirname "$py")")" "$(shell_quote "$HOME")"
