#!/usr/bin/env bash
# install.sh — make `pi` in your terminal able to run on the OpenAI Codex CLI (Ubuntu).
#
#   1. checks python3 / node 20+ / jq / pi / codex (installs pi + jq if missing; codex must be logged in)
#   2. registers the pure-LLM codex bridge (:18288) as an enabled systemd --user service + linger
#   3. registers the provider extension in ~/.pi/agent/settings.json and puts the codex-cli/*
#      models into pi's enabledModels scope when that scope exists (backup written first).
#      It does NOT steal pi's default provider — this backend is meant to sit next to
#      ../claude-code/pi-cli; pass --default if you want a bare `pi` to start on it.
...
[ "$(uname -s)" = "Linux" ] || die "this is the Ubuntu/Linux tree — on macOS run ../../macos/codex/install.sh"
...
have python3 || die "python3 not found — sudo apt install -y python3"
have node && [ "$(node -p 'process.versions.node.split(".")[0]')" -ge 20 ] || die "Node.js 20+ required"
if ! have jq; then info "installing jq…"; sudo apt-get install -y jq >/dev/null || die "could not install jq"; fi
...
have codex || die "'codex' not on PATH — npm i -g @openai/codex && codex login"
ok "codex $(codex --version 2>&1 | head -1)"
[ -f "${CODEX_HOME:-$HOME/.codex}/auth.json" ] \
  && ok "codex credentials present (${CODEX_HOME:-$HOME/.codex}/auth.json)" \
...
# enabledModels is pi's model SCOPE: when it exists, models outside it are ignored — including
# ours, so `--provider codex-cli` would silently fall back to another enabled model. Ask the
# running bridge which models it advertises (it discovers them from codex's own catalogue).
# NOTE: entries are exact references, `provider/id`. Globs are avoided on purpose — an id that
# itself contains a slash (opencode's do) will not match a single-star pattern.
...
ok "extension registered in $SETTINGS$([ "$SET_DEFAULT" -eq 1 ] && echo "; default provider = codex-cli, model $MODEL")"

info "smoke test…"
"$HERE/run.sh" test || warn "smoke test failed — is the bridge up? ./run.sh service-status | logs | journal"
echo
...
echo "     ./run.sh logs | journal | service-status | models | uninstall-service"
