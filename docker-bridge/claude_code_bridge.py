#!/usr/bin/env python3
"""chat-completions-compatible proxy for OpenAI-API clients -> Claude Code CLI.

This bridge accepts /health, /config, /v1/models and /v1/chat/completions, calls
an authenticated `claude -p`, and returns chat-completions responses to any
...
over a plain blocking/text response:
  ...
  * Session continuity  - each client conversation is mapped (by a hash of its
                          ...
                          cheap `cache_read` on later turns. Requires no client
...
# Claude Code model IDs advertised on /v1/models for discovery/UI (e.g. a client
...
CLAUDE_CODE_MODELS = [
    ...
    for m in os.getenv(
        "CLAUDE_CODE_MODELS",
        "claude-opus-5,claude-fable-5,claude-opus-4-8,claude-opus-4-7,claude-opus-4-6,claude-sonnet-5,claude-sonnet-4-6,claude-haiku-4-5,claude-haiku-4-5-20251001",
    ).split(",")
    if m.strip()
]
...
def _convo_key(messages: list[dict[str, Any]]) -> str:
    ...
    the client echoes back for our prior reply. The key over a request's full
...
def _usage_from_result(result_event: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        # Non-standard extensions a client can use for spend tracking.
                ...
                400,
                {
                    "error": {
                        "message": (
                            ...
                            "Configure a provider that can serve the selected model."
                        )
                    }
                },
            )
    ...
    def _handle_completion(
        ...
        for attempt in range(attempts):
            ...
            if attempt < attempts - 1:
                # Regenerate the session id before retrying. The failed attempt may
                # already have registered it with Claude Code, in which case reusing
                # it fails with "Session ID <uuid> is already in use" — which then
                # masks the real error from attempt 1.
                new_id = str(uuid.uuid4())
            ...
            for raw_line in proc.stdout:
                ...
                if etype == "system" and event.get("subtype") == "init":
                    ...
                    if event.get("is_error"):
                        # api_error_status is an int HTTP status; result carries the
                        # human-readable cause (e.g. "You've hit your monthly spend
                        # limit."). Keep both — the status alone is not actionable.
                        status = event.get("api_error_status")
                        detail = event.get("result")
                        error_text = (
                            f"claude api error {status}: {detail}"
                            if status is not None and detail
                            else (detail or (f"claude api error {status}" if status is not None else None))
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=os.getenv(
            ...
            "You are being called through a local OpenAI-compatible proxy. Use only explicitly allowed tools. "
