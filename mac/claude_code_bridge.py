...
def effort_from_request(request: dict) -> Optional[str]:
    """OpenAI ``reasoning_effort`` → ``claude --effort``. none/minimal → low; unknown → None (bridge default)."""
    raw = request.get("reasoning_effort")
    if raw is None and isinstance(request.get("reasoning"), dict):
...
def build_command(cfg: "BridgeConfig", model: str, prompt: str, system_prompt: str,
                  output_format: str, image_dir: Optional[str] = None, effort: Optional[str] = None) -> list[str]:
    cmd = [
        cfg.claude_bin,
        "--model", model if cfg.pass_model else cfg.default_model,
        "--effort", effort or cfg.effort,  # per-request reasoning_effort wins over the default
    ...
    if cfg.tools is not None:
        cmd += ["--tools", "" if cfg.tools.strip().lower() in ("none", '""', "''") else cfg.tools]
...
def run_blocking(cfg: "BridgeConfig", model: str, prompt: str, system_prompt: str,
                 image_dir: Optional[str] = None, effort: Optional[str] = None) -> dict[str, Any]:
    """Run claude to completion in JSON mode; return {'text','usage','cost_usd'}."""
    cmd = build_command(cfg, model, prompt, system_prompt, "json", image_dir, effort)
    proc = _popen(cmd, cfg)
    try:
    ...
    # `-p --output-format json` returned ONE object on older CLIs; since Claude Code
    # 2.1.x it returns an ARRAY of stream events (system/init, assistant, …, result).
    # Handle both: without the list branch the whole event array was returned verbatim
    # as the assistant's reply, and usage came back as zeros.
    try:
    ...
    if isinstance(payload, list):
        # The terminal `result` event carries the final text, usage and cost.
        for event in reversed(payload):
            if isinstance(event, dict) and event.get("type") == "result":
                if event.get("is_error"):
                    # 502 matches run_streaming's handling of an is_error result event.
                    raise ClaudeError(_safe_error_text(
                        event.get("result") or event.get("api_error_status")
                        or "claude reported an error"), 502)
                return {
                    "text": event.get("result", "") or "",
                    "usage": event.get("usage", {}) or {},
                    "cost_usd": event.get("total_cost_usd"),
                }
        # No result event (truncated run): fall back to concatenating assistant text.
        chunks: list[str] = []
        ...
        if chunks:
            return {"text": "".join(chunks).strip(), "usage": {}, "cost_usd": None}
    return {"text": out.strip(), "usage": {}, "cost_usd": None}


def run_streaming(cfg: "BridgeConfig", model: str, prompt: str, system_prompt: str,
                  image_dir: Optional[str] = None, effort: Optional[str] = None) -> Iterator[dict[str, Any]]:
    ...
    """
    cmd = build_command(cfg, model, prompt, system_prompt, "stream-json", image_dir, effort)
    proc = _popen(cmd, cfg)
    ...
    emitted_text = ""  # everything streamed so far (for turn separators)
    ...
    try:
        ...
        for line in proc.stdout:
            ...
            if etype == "assistant":
                for part in (event.get("message", {}) or {}).get("content", []) or []:
                    if isinstance(part, dict) and part.get("type") == "text" and part.get("text"):
                        # Text from successive internal turns (e.g. around MCP connector calls)
                        # would otherwise run together: "…connecting.Retrying.Yes —". Separate them.
                        text = part["text"]
...
class BridgeConfig:
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        # --tools passthrough: None = CLI default; "none" → --tools "" (pure LLM, no built-in tools/MCP).
        self.tools = args.tools if args.tools not in (None, "") else None
...
class BridgeHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        effort = effort_from_request(request)
        ...
        try:
            if request.get("stream") is True:
                self._handle_stream(model, prompt, system_prompt, image_dir, effort)
            else:
                self._handle_blocking(model, prompt, system_prompt, image_dir, effort)
        finally:
    ...
    def _handle_blocking(self, model: str, prompt: str, system_prompt: str,
                         image_dir: Optional[str] = None, effort: Optional[str] = None) -> None:
        started = time.time()
        try:
            res = run_blocking(self.cfg, model, prompt, system_prompt, image_dir, effort)
        ...
        print(f"[claude-code-bridge] model={model} effort={effort or self.cfg.effort} chars={len(prompt)} "
    ...
    def _handle_stream(self, model: str, prompt: str, system_prompt: str,
        ...
        try:
            ...
            usage: dict[str, Any] = {}
            cost = None
            for event in run_streaming(self.cfg, model, prompt, system_prompt, image_dir, effort):
                if "delta" in event:
            ...
            print(f"[claude-code-bridge] stream model={model} effort={effort or self.cfg.effort} chars={len(prompt)} "
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--tools", default=os.getenv("CLAUDE_CODE_TOOLS") or None,
                   help='claude --tools passthrough: "none" disables every built-in tool (pure LLM), "default" = all, or a list like "Bash,Read"')
...
def _selfcheck() -> None:
    ...
    _t = build_command(BridgeConfig(build_parser().parse_args(["--tools", "none"])), "claude-opus-5", "p", "", "json")
    assert "--tools" in _t and _t[_t.index("--tools") + 1] == "", _t
    assert "--tools" not in build_command(BridgeConfig(build_parser().parse_args([])), "claude-opus-5", "p", "", "json")
    assert "claude-fable-5-1" in DEFAULT_MODELS
    _e = build_command(BridgeConfig(build_parser().parse_args([])), "claude-opus-5", "p", "", "json", None, "low")
    assert _e[_e.index("--effort") + 1] == "low", _e
    assert effort_from_request({"reasoning_effort": "minimal"}) == "low" and effort_from_request({"reasoning_effort": "xhigh"}) == "xhigh"
    assert effort_from_request({"reasoning": {"effort": "high"}}) == "high" and effort_from_request({}) is None
    print("selfcheck ok")
