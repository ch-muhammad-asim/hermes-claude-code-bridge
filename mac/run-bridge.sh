...
LABEL="${CLAUDE_CODE_BRIDGE_LABEL:-com.hermes.claude-code-bridge}"   # launchd label / systemd unit stem (override for a 2nd instance)
...
build_argv() {
  ...
  # CLAUDE_CODE_TOOLS=none → `claude --tools ""` (pure LLM; used by ../pi-agents/macos/claude-code)
  if [ -n "${CLAUDE_CODE_TOOLS:-}" ]; then ARGV+=( --tools "$CLAUDE_CODE_TOOLS" ); fi
}
...
print_config() {
  ...
  echo "  tools:          allowed=${CLAUDE_CODE_ALLOWED_TOOLS:-*} disallowed=${CLAUDE_CODE_DISALLOWED_TOOLS:-(empty)} permission_mode=${CLAUDE_CODE_PERMISSION_MODE:-bypassPermissions} tools=${CLAUDE_CODE_TOOLS:-default}"
}
